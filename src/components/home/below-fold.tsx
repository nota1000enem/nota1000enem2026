...
export default function HomeBelowFold() {
  ...
  const [topSemana, setTopSemana] = useState<
    Array<{ nome: string; melhor_nota: number; avatar_url: string | null; estado: string | null; idade: number | null }>
  >([]);
  ...
  useEffect(() => {
    supabase.rpc("get_ranking_global").then(({ data }) => {
      if (data && Array.isArray(data))
        setTopSemana(
          (data as Array<{ nome: string; melhor_nota: number; avatar_url: string | null; estado: string | null; idade: number | null }>).slice(0, 3),
        );
    });
  }, []);
  ...
  async function handleBuy(planType: PlanType) {
    ...
    try {
      ...
      if (!w) window.location.href = res.init_point;
    } catch (e: unknown) {
      ...
      <section className="py-20">
        <div className="mx-auto max-w-5xl px-4 text-center">
          ...
          <div className="mt-8 grid gap-4 grid-cols-3">
            {(topSemana.length > 0
              ? topSemana.map((t, i) => ({
                  pos: `${i + 1}º`, nome: t.nome, nota: t.melhor_nota, avatar_url: t.avatar_url, estado: t.estado, idade: t.idade,
              ...
              : [0, 1, 2].map((i) => ({
                  pos: `${i + 1}º`, nome: "—", nota: 0, avatar_url: null as string | null, estado: null as string | null, idade: null as number | null,
                  color: i === 0 ? "text-yellow-400" : i === 1 ? "text-slate-300" : "text-orange-400",
                  ring: i === 0 ? "ring-yellow-400/60" : i === 1 ? "ring-slate-300/60" : "ring-orange-400/60",
                  glow: i === 0 ? "glow-blue" : "",
                }))
              ...
              <Card key={p.pos} className={`card-glass p-3 md:p-6 text-center ring-2 ${p.ring} ${p.glow}`}>
                ...
                <p className={`mt-2 text-[10px] md:text-xs uppercase tracking-wider font-semibold ${p.color}`}>{p.pos} lugar</p>
          ...
          {(() => {
            const planosArr = [
              { name: "ENEM Light", planType: "LIGHT" as PlanType, price: "19,90", oldPrice: null as string | null, periodo: "/mês", cta: "Começar Agora", popular: false,
                items: ["10 redações nota 1000 prontas","+ 50 BÔNUS incluso","15 redações por mês","Linguagens, Códigos e suas Tecnologias","Ciências da Natureza e suas Tecnologias","1.000 questões e simulados","Plano de Estudo com IA","PDF metodologia de estudos","Cronograma de 30 dias","Templates de redação nota 1000","Acesso básico IA","Grupo VIP + Network"] },
              { name: "ENEM Pro", planType: "PRO" as PlanType, price: "29,90", oldPrice: "49,90" as string | null, periodo: "/mês", cta: "Quero o Pro", popular: true,
                items: ["10 redações nota 1000 prontas","+ 50 BÔNUS incluso","30 redações por mês","Tudo do Light","Ciências Humanas e suas Tecnologias","Matemática e suas Tecnologias","As 4 áreas do ENEM liberadas","20 vídeo aulas","1.000 questões para passar","Simulados","Correção IA avançada","IA Professor Rígido","Plano de Estudo com IA","Repertórios automáticos","Cronograma inteligente","Grupo VIP + Network"] },
              { name: "Full Access ENEM", planType: "FULL" as PlanType, price: "44,90", oldPrice: "99,90", periodo: "/mês", cta: "Quero Acesso Total", popular: false,
                items: ["10 redações nota 1000 prontas","+ 50 BÔNUS incluso","60 redações por mês","Tudo do Pro","Redação completa","BÔNUS — 9 segredos para aprovação no vestibular","Correção IA ilimitada","1.000 questões avançadas","Vídeo aulas completas","Simulados ilimitados","Templates premium","Ranking de alunos","IA Professor Rígido","Plano de Estudo com IA","Repertórios automáticos","Estratégias de aprovação","Atualizações futuras","Grupo VIP + Network"] },
              { name: "Full Access ENEM Vitalício", planType: "VITALICIO" as PlanType, price: "499", oldPrice: null, periodo: "uma vez", cta: "Quero Vitalício", popular: false,
            ...
            const renderCard = (p: typeof planosArr[number]) => (
              <Card key={p.name} className={`relative flex h-full flex-col p-6 card-glass ${p.popular ? "card-gradient-border pt-9 glow-blue" : ""}`}>
                {p.popular && (
                  <Badge className="absolute top-2 left-1/2 -translate-x-1/2 btn-gradient-hot text-white shadow-md">MAIS VENDIDO</Badge>
                )}
              ...
              <>
                <div className="md:hidden">
                  <Carousel opts={{ align: "start" }} className="px-8">
                    <CarouselContent>
                      {planosArr.map((p) => (
                        <CarouselItem key={p.name} className="basis-[85%]">{renderCard(p)}</CarouselItem>
                      ))}
                    </CarouselContent>
                    <CarouselPrevious className="left-0" />
                    <CarouselNext className="right-0" />
